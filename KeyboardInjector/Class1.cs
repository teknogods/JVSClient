...
namespace KeyboardInjector
{
    public class KeyboardInject
    {
        ...
        INPUT[] inputs = new INPUT[0x28];
        INPUT[] inputs_out = new INPUT[0x28];
        uint inputcount = 0;
        uint output = 0;
        ...
        public void NullNormalInputs()
        {
            BitArray Digitals = new BitArray(0x28);
            // Player 1 Buttons
            // Z
            CreateInput(0x5A, Digitals[0x09]);
            // X
            CreateInput(0x58, Digitals[0x08]);
            // C
            CreateInput(0x43, Digitals[0x17]);
            // V
            CreateInput(0x56, Digitals[0x16]);
            // B
            CreateInput(0x42, Digitals[0x15]);
            // N
            CreateInput(0x4E, Digitals[0x14]);

            // Player 1 Joystick
            // UP
            CreateInput(VK_UP, Digitals[0x0D], true);
            // DOWN
            CreateInput(VK_DOWN, Digitals[0x0C], true);
            // LEFT
            CreateInput(VK_LEFT, Digitals[0x0B], true);
            // RIGHT
            CreateInput(VK_RIGHT, Digitals[0x0A], true);

            // Player 1 Special
            // Enter
            CreateInput(VK_RETURN, Digitals[0x07], true);
            // 5
            CreateInput(0x35, Digitals[0x0F]);
            // 8
            CreateInput(0x38, Digitals[0x0F]);

            // Player 2 Buttons
            // T
            CreateInput(0x54, Digitals[0x19]);
            // Y
            CreateInput(0x59, Digitals[0x18]);
            // U
            CreateInput(0x55, Digitals[0x27]);
            // G
            CreateInput(0x47, Digitals[0x26]);
            // H
            CreateInput(0x48, Digitals[0x25]);
            // J
            CreateInput(0x4A, Digitals[0x24]);
            // Player 2 Joystick
            // W
            CreateInput(0x57, Digitals[0x1D]);
            // S
            CreateInput(0x53, Digitals[0x1C]);
            // A
            CreateInput(0x41, Digitals[0x1B]);
            // D
            CreateInput(0x44, Digitals[0x1A]);
            // Player 2 Special
            // 2
            CreateInput(0x32, Digitals[0x1F]);

            // TEST
            CreateInput(VK_F2, Digitals[0x0E]);

            // Esc
            CreateInput(VK_ESCAPE, false);
        }

        public void CreateInputs(BitArray Digitals)
        {
            // Player 1 Buttons
            // Z
            CreateInput(0x5A, Digitals[0x09]);
            ...
            // Player 1 Special
            // Enter
            CreateInput(VK_RETURN, Digitals[0x0F], true);
            // 5
            CreateInput(0x35, Digitals[0x07]);
            // 8
            CreateInput(0x38, Digitals[0x07]);
            ...
            CreateInput(0x32, Digitals[0x1F]);
            //// 6
            //CreateInput(0x36, fastioButtons->CoinSlot2);
            //// 9
            //CreateInput(0x39, fastioButtons->Player2Service);
        }

        public void Initialize()
        {
            NullNormalInputs();
            SendInput(inputcount, inputs, INPUT.Size);
            hasInitialized = true;
            inputcount = 0;
            output = 0;
        }

        public void SendInputs(BitArray Digitals)
        {
            if (!hasInitialized)
                return;
            CreateInputs(Digitals);
            SendInput(output, inputs_out, INPUT.Size);
            inputcount = 0;
            output = 0;
        }
